...
const PopularShowsPage = ({ PopularShowsData }) => {
  const { id } = useParams();
  const itemId = parseInt(id);
  const item = PopularShowsData.find((item) => item.id === itemId);
  const {User} = UserAuth()
    ...
    <div className='text-white pb-96'>
    {
      item ? (
        <div className='md:pl-8 md:block flex flex-col pt-12'>
         ...
         <div className='md:w-1/2  flex flex-col absolute md:top-48 top-20 md:ml-16 ml-2 md:gap-6 '>
        ...
        <div className='flex pt-64 md:pt-5'>
        {
          ...
          <div className='border-2 md:px-3 w-6 md:w-12 md:py-3 ml-4 px-1 py-1 bg-slate-950 -mb-1 border-white rounded-full '>
            <img src={watchlists} className='md:w-5 md:h-5  h-3 w-5' />
          </div>
        </div>

          <div className='w-full flex md:flex-row flex-col gap-3 md:gap-2 pt-2 md:text-bold max-[767px]:text-sm  text-lg'>
